 ...
 * with Vapi faked at the `fetch` boundary. Nothing here talks to the network.
 *
...
const CALL_ID = 'call_test_123';
const CONTROL_URL = 'https://control.vapi.test/call_test_123';
...
function post(message: Record<string, unknown>, secret = SECRET) {
  return app.inject({
    ...
    payload: { message: { call: { id: CALL_ID }, ...message } },
  });
}
...
describe('the happy path', () => {
  it('runs interview → arm → recipient joins → handoff → summary', async () => {
    ...
    expect(store.getCall(CALL_ID)?.phase).toBe('interviewing');

    // --- User starts merging ---
    const armed = await toolCall('arm_for_merge', { userFirstName: 'Sam' });
    expect(armed.statusCode).toBe(200);
    expect(store.getCall(CALL_ID)?.phase).toBe('awaiting_recipient');
    expect(store.getCall(CALL_ID)?.userFirstName).toBe('Sam');
    ...
    const handoff = await app.inject({
      ...
      payload: {
        message: {
          ...
          call: { id: CALL_ID },
          variableValues: intent,
        },
      },
    });

    const destination = handoff.json().destination;
    ...
    const record = store.getCall(CALL_ID)!;
    ...
    const finished = store.getCall(CALL_ID)!;
...
describe('the backstop', () => {
  it('speaks the disclosure when the model misses the recipient arriving', async () => {
    ...
    expect(controlCalls).toEqual([
      { type: 'say', content: buildDisclosure('Sam'), endCallAfterSpoken: false },
    ]);
    expect(store.getCall(CALL_ID)?.backstopFired).toBe(true);
    expect(store.getCall(CALL_ID)?.handoffTrigger).toBe('server_backstop');
  });
  ...
  it('stays silent before arming — "hello" is ordinary mid-interview', async () => {
    ...
    // removed.
    store.upsertCall(CALL_ID, { userFirstName: 'Sam' });
...
describe('arming refuses without a usable name', () => {
  it('tells the model to go get the name rather than arming', async () => {
    ...
    expect(response.json().results[0].result).toMatch(/don't have the user's/i);
    ...
    const response = await app.inject({
      ...
      payload: {
        message: {
          type: 'handoff-destination-request',
          call: { id: CALL_ID },
          variableValues: { userFirstName: 'Sam' }, // missing everything else
        },
      },
    });

    expect(response.json().destination).toBeUndefined();
    expect(response.json().error).toMatch(/refusing handoff/i);

    // Never hand a recipient an assistant that doesn't know its boundaries.
    expect(store.getCall(CALL_ID)?.phase).not.toBe('delegating');
...
describe('hard blocks', () => {
  it('returns the refusal verbatim and marks the call blocked', async () => {
    ...
    expect(response.json().results[0].result).toMatch(/800-799-7233/);
    expect(store.getCall(CALL_ID)?.phase).toBe('blocked');
  });
});

describe('webhook auth', () => {
  it('rejects a wrong secret without touching call state', async () => {
...
describe('recording', () => {
  it('is disabled on both assistants until consent is settled', async () => {
    ...
    expect(interviewAssistant('https://x.test').artifactPlan.recordingEnabled).toBe(
      false,
    );
    expect(
